...
FALSE_LAMBDA_VEC = np.array(list(FALSE_ALERT_LAMBDAS.values()), dtype=np.float64)


def _calculate_undetected_prob(
    N_vec: NDArray[np.int32],
    S_matrix: NDArray[np.int32],
    alpha_vec: NDArray[np.int32],
    M_vec: NDArray[np.int32],
):
...
class SystemState:
    ...
    def state_update_attacker(self, attack_array: NDArray[np.bool_]):
        ...
        :return: None
        """
        self.alerts_due_attack.fill(0)  # Required?
    ...
    def state_update_defender(self, investigation_array: NDArray[np.int32]):
        ...
        p_undetected = _calculate_undetected_prob(
            self.uninvestigated_alerts,
            self.alerts_due_attack,
            investigation_array,
            self.attack_mounted.astype(int),
        )
...
if __name__ == "__main__":
    ...
    print(
        state.state_update_defender(
            np.array([500, 500, state.uninvestigated_alerts[2], 500, 500, 500, 500])
        )
    )
    state.reset()
    print(state)
